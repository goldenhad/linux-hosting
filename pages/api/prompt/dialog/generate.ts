  ...
  if( req.method == "POST" ){
    ...
    if( token ){
      ...
      if( data.name && data.personal && data.dialog && data.continue && data.address && data.style && data.order && data.emotions && data.length ){
        ...
        const prompt = `Mein Name ist ${data.name}. Ich bin ${data.personal}. Ich habe folgenden E-Mail-Dialog erhalten ${data.dialog}. 
        Schreibe in meinem Namen eine Antwort auf den bisherigen Dialog, der dabei folgende Punkte berücksichtigt: "${data.continue}". 
        Schreibe deine Antwort in der ${data.address}-Form. Der allgemeine Stil deiner Antwort sollte dabei ${data.style.toString()} sein. 
