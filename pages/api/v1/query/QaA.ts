  ...
  if(req.method == "POST"){
    ...
    if(aid && query){
      ...
      validateApiKey(apikey).then(async ({ user }) => {
        // If the key is valid...
        // Parse the given query as json object
        const parsedQuery = query;
        const companyReq = await firestore.doc(`/Company/${user.Company}`).get();
        const companyData = companyReq.data() as Company;
        ...
        const roleData = assistantData.data() as Role;


        if(liveAssistantData && companyData && roleData){
          if(liveAssistantData.blocks.length > 0){
            const inputBlock = liveAssistantData.blocks[0] as InputBlock;

            if(inputBlock.type == AssistantType.QAA){
              // Init the index object
              let index = undefined;
              
              let decryptedProfiles = [];
              ...
              for( let i = 0; i < user.profiles.length; i++ ) {
                ...
                try {
                  ...
                  const decoded = decryptProfile(user.profiles[i], user.salt);
              ...
              decryptedProfiles = profilearr;

              let replaced = inputBlock.prompt;

              console.log(replaced);

              inputBlock.inputColumns.forEach((column: AssistantInputColumn) => {
                column.inputs.forEach((inputobj) => {
                  const cleanedValued = query[inputobj.key];
                  if(inputobj.type == AssistantInputType.PROFILE){
                    const profile: Profile = decryptedProfiles.find( ( singleProfile: Profile ) => {
                      return singleProfile.name == query.profile;
                    });

                    if(profile){
                      let profileInformation = `Mein Name ist ${user.firstname} ${user.lastname}. Ich bin ${profile.settings.personal}`;
                      if(roleData.isCompany){
                        const companyinfo = `Ich arbeite für ${companyData.name}. Wir beschäftigen uns mit: ${companyData.settings.background}`;
              ...
              const chatEngine = new ContextChatEngine({ retriever, chatModel: llm , chatHistory: [] });

              // Define the response string
              let resp = "";

              // Query the chat engine for a response
              const response = chatEngine.chat({ message: replaced, stream: true });
              ...
              response.then(async (iter) => {
                ...
                if(companyData && calculationData.data() && invoiceData.data() && assistantData.data()){
