...
export default async function handler( req: NextApiRequest, res: NextApiResponse<ResponseData> ) {
  if(req.method == "POST"){
    ...
    if(aid && query && apikey && messages?.length >= 0){

      // validate the api key
      validateApiKey(apikey).then(async ({ companyId }) => {
        // If the key is valid...
          ...
          * */
          ...
          if(companyData.data() && calculationData.data() && invoiceData.data() && assistantData.data()){
            ...
            if(assistantInputBlock && assistantInputBlock.type != undefined){
              ...
              return res.status( 500 ).send( { errorcode: 4, message: "Assistant configuration invalid!" } );
            }
